...
describe('Auth routes', () => {
  describe('POST /api/v1/auth/register', () => {
    ...
    test('should return 201 and successfully register user if request data is ok', async () => {
      const res = await request(app).post('/api/v1/auth/register').send(newUser).expect(status.CREATED);
    ...
    test('should return 400 error if email is invalid', async () => {
      ...
      await request(app).post('/api/v1/auth/register').send(newUser).expect(status.BAD_REQUEST);
    });
  ...
  describe('POST /api/v1/auth/login', () => {
    ...
    test('should return 401 error if password is wrong', async () => {
      ...
      const res = await request(app).post('/api/v1/auth/login').send(loginCredentials).expect(status.UNAUTHORIZED);
